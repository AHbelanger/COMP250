...
//            System.out.println(myBasket);
        ...
        try{
            Customer cust  = new Customer("Alice",2000);
...
//            System.out.println(cust.getBalance());
			...
			cust.addToBasket(new Jam("Pink Jam", 1, 99));
			cust.addToBasket(new Egg("white egg", 1, 100));
			cust.addToBasket(new SeasonalFruit("Orange", 2, 50));
			cust.addToBasket(new Egg("brown egg", 9, 12));
			cust.addToBasket(new SeasonalFruit("Apple", 5, 60));

			
			cust.removeFromBasket(new Fruit("Yellow Berry", 1.5, 40));
			cust.removeFromBasket(new Egg("white egg", 1, 100));
            System.out.println("Balance Before:" + cust.getBalance());
            System.out.println("Basket cost:" + cust.getBasket().getTotalCost());
            System.out.print(cust.checkOut());
    ...
    private static void fruitTest(){
        try{
            Fruit myFruits = new Fruit("Asian Pear",1.25,529);
            int costOfmyFruits = myFruits.getCost();
            if(costOfmyFruits != 661){
